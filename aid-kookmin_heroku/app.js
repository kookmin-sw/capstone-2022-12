...
app.listen(process.env.PORT || 80, function () {
    ...
    schedule.scheduleJob(' 0 1 17 * * 6', function () {
        handleDisconnect();
        ...
        // console.log(files);
        client.query('SELECT DISTINCT user_Serial_Number FROM log WHERE DATEDIFF(NOW(), date) > 13', (err, data) => {
            try {
                if (err | data.length == 0) {
                    console.log('DB query err or no user more than 2 weeks');
                    throw (err);

                }
                console.log(data);
                for (var i = 0; i < data.length; i++) {
                    console.log("n번째 사용자 입니다");
